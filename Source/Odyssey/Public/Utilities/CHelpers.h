...
public:
	...
	template <typename T>
	static void CreateComponent(AActor* InActor, TObjectPtr<T>& OutComponent, FName InName, USceneComponent* InParent = nullptr, FName InSocketName = NAME_None)
	{
        OutComponent = InActor->CreateDefaultSubobject<T>(InName);
	...
	static T* FindActor(UWorld* InWorld)
	{
		...
		{
			if (!!actor && actor->IsA<T>()) return Cast<T>(actor);
		}
	...
	static void FindActors(UWorld* InWorld, TArray<T*>& OutActors)
	{
		for (AActor* actor : InWorld->GetCurrentLevel()->Actors)
		{
			if (!!actor && actor->IsA<T>()) OutActors.Add(Cast<T>(actor));
		}
	}
